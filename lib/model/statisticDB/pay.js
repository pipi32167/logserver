...
exp.createPayInfo = P.coroutine(function* createPayInfo(date) {
  ...
  let args = {
    date: date,
    pay: new BitSet(consts.DEFAULT_USER_BIT_MAP_SIZE), //支付日期为date的用户
    registerPay: new BitSet(consts.DEFAULT_USER_BIT_MAP_SIZE), //注册日期为date，并支付过的用户
    firstPay: new BitSet(consts.DEFAULT_USER_BIT_MAP_SIZE), //首次支付日期为date的用户
    secondPay: new BitSet(consts.DEFAULT_USER_BIT_MAP_SIZE), //第二次支付日期为date的用户
    totalPrice: 0, //日期为date的总收入
    totalDiamond: 0, //日期为date的总购买钻石数
  };
  payInfos[date] = args;
...
exp.queryPayRate = P.coroutine(function* queryPayRate(beginTime, endTime) {
  ...
  let registerCount = yield remainRate.queryRegisterCount(beginTime);
  let payCount = 0;
