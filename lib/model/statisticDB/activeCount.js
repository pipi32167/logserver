...
exp.queryActiveCount = P.coroutine(function* queryActiveCount(date) {
  ...
  let remainInfo = yield remainRate.queryRemainInfo(date);

  return activeCountInfo.active.getCardinality() - activeCountInfo.active.and(remainInfo.register).getCardinality();
})