...
let remainRate = exp.remainRate = exp.remain = app.require('lib/model/statisticDB/remainRate');
...
exp.clearDBs = function(dbs) {

  let sql = dbs.map(function(elem) {
    console.log(elem, exp[elem]);
    exp[elem].clear();
    return `TRUNCATE ${STATISTIC_DB_NAME}.${elem};`;
  }).join('');
  return mysqlUtil.query(sql, []);
}
...
exp.repairDB = P.coroutine(function* repairDB(serverId) {
  ...
  // yield exp.removeDB();
  yield exp.clearDBs(['activeCount', 'pay', 'remain']);
  ...
  let datas = yield logDB.queryData({
    ...
    type: [consts.LogType.REGISTER, consts.LogType.LOGIN, consts.LogType.IAP],
    serverId: serverId,
  });
...
// exp.repairDB2 = P.coroutine(function* repairDB2(serverId) {
//   var redisConfig = app.require('config/redis.json').log_queue;

//   var queue = P.promisifyAll(new NR.queue({
//     connection: redisConfig
//   }, {}));

//   yield queue.connectAsync();
...
//     beginTime: new Date('2016-01-01'),
//     endTime: new Date('2037-01-01'),
//     type: [consts.LogType.REGISTER, consts.LogType.LOGIN],
//     serverId: serverId,
//   });

//   logger.debug('repairDB len:', datas.length);
//   for (var i = 0; i < datas.length; i++) {
//     yield queue.enqueueAsync('log', 'import', JSON.parse(datas[i].detail));
//   }

//   // let remainCount;
//   // do {
//   //   yield P.delay(1000);
//   //   remainCount = yield queue.length();
//   // } while (remainCount > 0);

//   yield queue.endAsync();

//   // logger.debug('remain count:', queue.length());
// })