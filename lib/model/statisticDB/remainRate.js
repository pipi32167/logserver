...
exp.init = P.coroutine(function* init() {
  ...
  sql = `SELECT * FROM ${STATISTIC_DB_NAME}.remain`;

  let results = yield mysqlUtil.query(sql, []);
  for (var i = 0; i < results.length; i++) {
    let elem = results[i];
    elem.register = new BitSet(elem.register);
    elem.login = new BitSet(elem.login);
    remainInfos[elem.date] = elem;
  }
});
...
exp.register = function(userId, time) {
  ...
  return onebyone.addAsync(P.coroutine(function*(cb) {
    ...
    // console.log('login', offset, time);
    if (remainInfo.register.get(offset)) {
      return cb();
    }
    // logger.debug('register', remainInfo.register.getCardinality());
...
exp.queryRemainRate = P.coroutine(function* queryRemainRate(date, days) {
  ...
  return (remainCount / registerCount).toFixed(2);
})

exp.queryRegisterCount = P.coroutine(function* queryRegisterCount(date) {

  date = new Date(date).clearTime();

  let remainInfo = yield exp.queryRemainInfo(date);

  return remainInfo.register.getCardinality();
})

exp.queryLoginCount = P.coroutine(function* queryLoginCount(date) {

  date = new Date(date).clearTime();

  let remainInfo = yield exp.queryRemainInfo(date);

  let login = remainInfo.register.or(remainInfo.login);
  return login.getCardinality();
})