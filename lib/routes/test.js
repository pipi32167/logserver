...
module.exports = function(router) {
  ...
  router.post('/test/insert', routeUtil.createRoute(P.coroutine(function*() {
    ...
    let beginUserId = msg.beginUserId && parseInt(msg.beginUserId, 10) || consts.USER_ID_BEGIN;

    yield P.race(_.range(0, msg.count).map(function(elem) {

      let userId = !msg.random ? (beginUserId + elem) : _.random(beginUserId, beginUserId + msg.count - 1);
  ...
  router.post('/test/repair', routeUtil.createRoute(P.coroutine(function*() {

    yield statisticDB.repairDB(this.request.body.serverId);
  })));

  router.post('/test/repair2', routeUtil.createRoute(P.coroutine(function*() {

    yield statisticDB.repairDB2(this.request.body.serverId);
  })));
