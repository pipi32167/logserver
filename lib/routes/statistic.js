...
module.exports = function(router) {
  ...
  router.get('/statistic/common', routeUtil.createRoute(P.coroutine(function*() {
    ...
    for (var i = new Date(msg.beginTime); i <= msg.endTime; i = i.addDays(1)) {
      ...
      let res = {
        ...
        payRate: yield statisticDB.pay.queryPayRate(date, new Date(msg.endTime)),
  ...
  router.get('/statistic/queryOnlineCount', routeUtil.createRoute(P.coroutine(function*() {
    ...
    msg.beginTime = new Date(msg.beginTime);
    msg.endTime = new Date(msg.endTime);
    let res = yield statisticDB.onlineCount.queryOnlineCount(msg.beginTime, msg.endTime);
    return {
      results: res,
    }
  })));
